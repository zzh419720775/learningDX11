...
std::optional<int> Window::ProcessMessages() noexcept
{
	...
	{
		//check for quit becaust peekMessages does not signal this via return val
...
LRESULT CALLBACK Window::HandleMsgSetup(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) noexcept
{
	...
	if (msg == WM_NCCREATE) {
		...
		Window* const pWhd = static_cast<Window*>(pCreate->lpCreateParams);
		//set WinAPI-managed user data to store ptr to window instance
		SetWindowLongPtr(hWnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(pWhd));
		//set message proc to normal(non setup)hander now that setup is finished
		SetWindowLongPtr(hWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(&Window::HandleMsgTrunk));
		//forward message to window instance handler
		return pWhd->HandleMsg(hWnd, msg, wParam, lParam);
	}
